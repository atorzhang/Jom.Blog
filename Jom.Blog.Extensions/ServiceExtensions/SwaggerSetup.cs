            ...
            services.AddSwaggerGen(c =>
            {
                ...
                typeof(ApiVersions).GetEnumNames().ToList().ForEach(version =>
                {
                    c.SwaggerDoc(version, new OpenApiInfo
                    {
                        ...
                        Contact = new OpenApiContact { Name = ApiName, Email = "Jom.Blog@xxx.com", Url = new Uri("https://neters.club") },
                ...
                {
                    //这个就是刚刚配置的xml文件名
                    var xmlPath = Path.Combine(basePath, "Jom.Blog.xml");
                    //默认的第二个参数是false，这个是controller的注释，记得修改
                    c.IncludeXmlComments(xmlPath, true);

                    //这个就是Model层的xml文件名
                    var xmlModelPath = Path.Combine(basePath, "Jom.Blog.Model.xml");
                    c.IncludeXmlComments(xmlModelPath);
                }
                catch (Exception ex)
                {
                    log.Error("Jom.Blog.xml和Jom.Blog.Model.xml 丢失，请检查并拷贝。\n" + ex.Message);
                }
